...
本仓库已实现一个从零构建的 Python + Pygame 版本。实现重点是可运行的多主机 MVP：单机伪 3D 水族箱动画、UDP 局域网发现、手动校准相邻屏幕、鱼跨屏移交、状态面板、基础水声音效和 headless 测试。

任务书原文提出“三维运动动画”，功能表同时允许“2D 或 3D 游鱼动画”。本项目采用 Pygame 二维绘制，并通过 `depth` 缩放、阴影、旋转和层次排序形成伪 3D 视觉效果，作为课程允许的 2D/3D 动画实现取舍。
